...
\section{Déploiement sur Google Colab}

Faute de GPU local suffisant, le système est déployé sur \textbf{Google Colab} en
exploitant un GPU \textbf{NVIDIA A100}, qui dispose d'une mémoire vidéo amplement
suffisante pour charger le modèle en précision \texttt{float16} complète (environ
16~Go). La procédure de déploiement, automatisée par un \textit{notebook} dédié
(\texttt{Qwen2\_Audio\_Colab.ipynb}), se déroule en quelques étapes :
...
\begin{enumerate}
  ...
  \item Installation des dépendances (\texttt{transformers}, \texttt{accelerate},
        \texttt{librosa}, \texttt{ffmpeg}, FastAPI) sans réinstaller PyTorch, déjà
        fourni avec CUDA par Colab.
...
Afin de prendre en charge des GPU plus modestes (par exemple un Colab T4 d'environ
15~Go, insuffisant pour la précision \texttt{float16}), le chargement du modèle
accepte une \textbf{quantification optionnelle} en 4 bits ou 8 bits via la
bibliothèque \texttt{bitsandbytes}, activable par les variables d'environnement
\texttt{LOAD\_IN\_4BIT} et \texttt{LOAD\_IN\_8BIT}.
...
\begin{itemize}
  ...
  \item \textbf{Ressources mémoire.} Le modèle Qwen2-Audio-7B-Instruct requiert environ
        16 Go de VRAM en précision float16, ce qui a motivé le déploiement sur un GPU
        A100 via Google Colab, ainsi que l'ajout d'une quantification optionnelle (4 ou
        8 bits) pour les GPU de moindre capacité.
